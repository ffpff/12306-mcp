...
import { parseRawCookies, formatCookies, make12306Request, logDebug } from './utils.js';
...
server.tool('get-tickets', '查询12306余票信息。', {
    ...
    try {
        logDebug('开始发送请求...');
        const queryResponse = await make12306Request(queryUrl, queryParams, headers);
...
server.tool('get-train-route-stations', '查询列车途径车站信息。', {
    ...
    const queryResponse = await make12306Request(queryUrl, queryParams, { Cookie: formatCookies(USER_COOKIES) });
...
server.tool('one-click-order', '简化版一键下单流程，封装了下单所需的所有步骤', {
    ...
    try {
        ...
        process.stderr.write(`[ERROR] 一键下单工具出错: ${error.message}\n`);
        process.stderr.write(`[ERROR] 错误堆栈: ${error.stack}\n`);
        return {

...
export async function make12306PostRequest<T>(
  ...
  if (headers.Cookie || headers.cookie) {
    ...
    const cookieStr = formatCookies(data.cookies as Record<string, string>);
    mergedHeaders['Cookie' as keyof typeof mergedHeaders] = cookieStr;
 ...
 * 通用的12306 GET请求工具函数
 */
export async function make12306GetRequest<T>(
  url: string,
